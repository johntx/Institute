...
<body>
	<p>Docente: <b>{{$people->nombrecompleto()}}</b> Biometrico: <b>{{$tickeos->first()->biometric->nombre}}</b></p>
	<table style="border: 1px solid black;">
		...
		<tbody style="border-top: 1px solid black;">
			<tr>
				...
				<?php $ni=0; if(count($tickeo1)>0){
					$Fecha1=\Carbon\Carbon::parse($fecha_inicio);
					$Fecha2=\Carbon\Carbon::parse($tickeo1->first()['fecha']);
					if($Fecha1->format('H:i')>$Fecha2->format('H:i')){$intervalo='0 minutos'; $ni = 0;} else {
					$intervalo = $Fecha1->diff($Fecha2);
					$ni = $intervalo->format('%i');
					$intervalo = $intervalo->format('%i minutos'); }
				}else{ $intervalo=''; } ?>
			...
			<tr>
				...
				<?php $ni2=0; if(count($tickeo2)>0){
					$Fecha1=\Carbon\Carbon::parse($fecha_fin);
					$Fecha2=\Carbon\Carbon::parse($tickeo2->first()['fecha']);
					if($Fecha1->format('H:i')<$Fecha2->format('H:i')){$intervalo2='0 minutos'; $ni2 = 0;} else {
					$intervalo2 = $Fecha2->diff($Fecha1);
					$ni2 = $intervalo2->format('%i');
					$intervalo2 = $intervalo2->format('%i minutos'); }
				}else{ $intervalo2=''; } ?>
