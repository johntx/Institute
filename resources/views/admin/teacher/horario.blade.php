...
<div class="panel-body">
	<table class="sch">
		<tbody>
			<tr>
				<th colspan="7">HORARIO {{$te->nombrecompleto()}}</th>
			</tr>
			<tr>
			...
			<tr h="{{$key+1}}" @if ($key%2==0 && $key<10) hmas="hmas" @elseif ($key%2!=0 && $key>12 && $key<22) hmas="hmas" @endif @if ($key>7 && $key<13 || $key>20)  mid="mid" @endif>
				...
				?>
				<td x="{{$x}}">
					<div class="sch_hour" hora='{{$hour['id']}}' size="{{$hour['periodos']}}" carrera="{{$hour['carrera']}}" fecha="{{Jenssegers\Date\Date::parse($hour['fecha'])->format('j M Y')}}" asignatura="<b>{{$hour['asignatura']}}</b>" aula="{{$hour['aula']}} {{$hour['piso']}}">{{$hour['asignatura']}}</div>
				</td>
				@endif
				@endforeach
			</tr>
